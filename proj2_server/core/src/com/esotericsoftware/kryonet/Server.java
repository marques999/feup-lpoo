...
public class Server implements EndPoint
{
	...
	public void update(int timeout) throws IOException
	{
		...
		synchronized (updateLock)
		{ // Blocks to avoid a select while the selector is used to bind the
			// server connection.
		}
					...
					try
					{
						...
						{ // Must be a TCP read or write operation.
							...
							if ((ops & SelectionKey.OP_READ) == SelectionKey.OP_READ)
							{
								...
								catch (IOException ex)
								{
									if (TRACE)
									{
										trace("kryonet", "Unable to read TCP from: "
												+ fromConnection, ex);
									}
									else if (DEBUG)
									{
								...
								{
									if (ERROR)
										error("kryonet", "Error reading TCP from connection: "
												+ fromConnection, ex);
									fromConnection.close();
								}
							}
							if ((ops & SelectionKey.OP_WRITE) == SelectionKey.OP_WRITE)
							{
								...
								catch (IOException ex)
								{
									if (TRACE)
									{
										trace("kryonet", "Unable to write TCP to connection: "
												+ fromConnection, ex);
									}
									else if (DEBUG)
									{
						...
						catch (KryoNetException ex)
						{
							if (WARN)
							{
								...
								{
									if (ERROR)
										error("kryonet", "Error reading UDP from connection: "
												+ fromConnection, ex);
								}
								else
									warn("kryonet", "Error reading UDP from unregistered address: "
											+ fromAddress, ex);
							}
							continue;
						}
						...
						if (object instanceof FrameworkMessage)
						{
							if (object instanceof RegisterUDP)
							{
								...
								if (connection != null)
								{
									...
									if (DEBUG)
										debug("kryonet", "Port "
												+ udp.datagramChannel.socket().getLocalPort()
												+ "/UDP connected to: "
												+ fromAddress);
							...
							if (object instanceof DiscoverHost)
							{
								try
								{
									...
									if (DEBUG && responseSent)
										debug("kryonet", "Responded to host discovery from: "
												+ fromAddress);
								}
								catch (IOException ex)
								{
						...
						if (DEBUG)
							debug("kryonet", "Ignoring UDP from unregistered address: "
									+ fromAddress);
					}
